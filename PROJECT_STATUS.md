...
- Recommended import format: one sentence per line, `sentence | translation`, `#` for comments. Every import opens a preview (`src/library-import.js`, tested in `tests/library-import.test.js`): create a new library or append to an existing one (same file name → append is preselected); duplicates are matched ignoring case and spacing, and their translations can be merged, kept or replaced; columns can be swapped. Libraries export back to the same txt format. Google users can import a Google Sheet: paste its link, confirm it in Google's Picker (grants `drive.file` access to that one file), choose the sentence/translation columns and header row; the library keeps the sheet link (synced as a Drive appProperty) and 「从表格更新」 re-reads and merges it. Needs the Picker + Sheets APIs and a referrer-restricted API key (`deploy/README.md`).
